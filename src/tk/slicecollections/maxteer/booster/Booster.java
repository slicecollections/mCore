...
public class Booster {
  ...
  public static Booster parseBooster(BoosterType type, String toParse) {
  ...
  public static boolean setNetworkBooster(String id, Profile profile, Booster booster) {
    ...
    Database.getInstance().execute("UPDATE `mCoreNetworkBooster` SET `booster` = ?, `multiplier` = ?, `expires` = ? WHERE `id` = ?", nb.getBooster(), nb.getMultiplier(),
        nb.getExpires(), id);
    CACHE.put(id, nb);
    return true;
  }

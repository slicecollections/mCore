...
public class MenuProfile extends PlayerMenu {
  ...
  public MenuProfile(Profile profile) {
    ...
    this.setItem(10,
        BukkitUtils.putProfileOnSkull(this.player,
            BukkitUtils.deserializeItemStack(
                "SKULL_ITEM:3 : 1 : nome>&aInformações Pessoais : desc>&fRank: " + Role.getRoleByName(profile.getDataContainer("mCoreProfile", "role").getAsString()).getName()
                    + "\n \n&fCadastrado: &7" + SDF.format(profile.getDataContainer("mCoreProfile", "created").getAsLong()) + "\n&fÚltimo acesso: &7"
                    + SDF.format(profile.getDataContainer("mCoreProfile", "lastlogin").getAsLong()))));
    ...
    this.setItem(11, BukkitUtils
        .deserializeItemStack("PAPER : 1 : nome>&aEstatísticas : desc>&7Visualize as suas estatísticas de\n&7cada Minigame do nosso servidor.\n \n&eClique para ver as estatísticas!"));

package tk.slicecollections.maxteer.utils.enums;

import org.bukkit.Material;
import tk.slicecollections.maxteer.libraries.MinecraftVersion;

/**
 * Armazena todos os Materiais da 1.8.8 e converte para 1.13 caso necessário para evitar
 * incompatibilidades, ao utilizar um Material não modificavél por configurações é recomendado
 * utilizar esta classe para requisitar o {@link Material}.
 *
 * @author Maxter
 */
public enum EnumMaterial {
  AIR("AIR"),
  STONE("STONE"),
  GRASS("GRASS_BLOCK"),
  DIRT("DIRT"),
  COBBLESTONE("COBBLESTONE"),
  WOOD("OAK_PLANKS"),
  SAPLING("OAK_SAPLING"),
  BEDROCK("BEDROCK"),
  WATER("WATER"),
  STATIONARY_WATER("WATER"),
  LAVA("LAVA"),
  STATIONARY_LAVA("LAVA"),
  SAND("SAND"),
  GRAVEL("GRAVEL"),
  GOLD_ORE("GOLD_ORE"),
  IRON_ORE("IRON_ORE"),
  COAL_ORE("COAL_ORE"),
  LOG("OAK_LOG"),
  LEAVES("OAK_LEAVES"),
  SPONGE("SPONGE"),
  GLASS("GLASS"),
  LAPIS_ORE("LAPIS_ORE"),
  LAPIS_BLOCK("LAPIS_BLOCK"),
  DISPENSER("DISPENSER"),
  SANDSTONE("SANDSTONE"),
  NOTE_BLOCK("NOTE_BLOCK"),
  BED_BLOCK("RED_BED"),
  POWERED_RAIL("POWERED_RAIL"),
  DETECTOR_RAIL("DETECTOR_RAIL"),
  PISTON_STICKY_BASE("STICKY_PISTON"),
  WEB("COBWEB"),
  LONG_GRASS("DEAD_BUSH"),
  DEAD_BUSH("DEAD_BUSH"),
  PISTON_BASE("PISTON"),
  PISTON_EXTENSION("PISTON_HEAD"),
  WOOL("WHITE_WOOL"),
  PISTON_MOVING_PIECE("MOVING_PISTON"),
  YELLOW_FLOWER("DANDELION"),
  RED_ROSE("POPPY"),
  BROWN_MUSHROOM("BROWN_MUSHROOM"),
  RED_MUSHROOM("RED_MUSHROOM"),
  GOLD_BLOCK("GOLD_BLOCK"),
  IRON_BLOCK("IRON_BLOCK"),
  DOUBLE_STEP("STONE_SLAB"),
  STEP("STONE_SLAB"),
  BRICK("BRICKS"),
  TNT("TNT"),
  BOOKSHELF("BOOKSHELF"),
  MOSSY_COBBLESTONE("MOSSY_COBBLESTONE"),
  OBSIDIAN("OBSIDIAN"),
  TORCH("TORCH"),
  FIRE("FIRE"),
  MOB_SPAWNER("SPAWNER"),
  WOOD_STAIRS("OAK_STAIRS"),
  CHEST("CHEST"),
  REDSTONE_WIRE("REDSTONE_WIRE"),
  DIAMOND_ORE("DIAMOND_ORE"),
  DIAMOND_BLOCK("DIAMOND_BLOCK"),
  WORKBENCH("CRAFTING_TABLE"),
  CROPS("WHEAT"),
  SOIL("FARMLAND"),
  FURNACE("FURNACE"),
  BURNING_FURNACE("AIR"),
  SIGN_POST("SIGN"),
  WOODEN_DOOR("OAK_DOOR"),
  LADDER("LADDER"),
  RAILS("RAIL"),
  COBBLESTONE_STAIRS("COBBLESTONE_STAIRS"),
  WALL_SIGN("AIR"),
  LEVER("LEVER"),
  STONE_PLATE("STONE_PRESSURE_PLATE"),
  IRON_DOOR_BLOCK("IRON_DOOR"),
  WOOD_PLATE("OAK_PRESSURE_PLATE"),
  REDSTONE_ORE("REDSTONE_ORE"),
  GLOWING_REDSTONE_ORE("REDSTONE_ORE"),
  REDSTONE_TORCH_OFF("AIR"),
  REDSTONE_TORCH_ON("REDSTONE_TORCH"),
  STONE_BUTTON("STONE_BUTTON"),
  SNOW("SNOW"),
  ICE("ICE"),
  SNOW_BLOCK("SNOW_BLOCK"),
  CACTUS("CACTUS"),
  CLAY("CLAY"),
  SUGAR_CANE_BLOCK("SUGAR_CANE"),
  JUKEBOX("JUKEBOX"),
  FENCE("OAK_FENCE"),
  PUMPKIN("CARVED_PUMPKIN"),
  NETHERRACK("NETHERRACK"),
  SOUL_SAND("SOUL_SAND"),
  GLOWSTONE("GLOWSTONE"),
  PORTAL("NETHER_PORTAL"),
  JACK_O_LANTERN("JACK_O_LANTERN"),
  CAKE_BLOCK("CAKE"),
  DIODE_BLOCK_OFF("REPEATER"),
  DIODE_BLOCK_ON("REPEATER"),
  STAINED_GLASS("WHITE_STAINED_GLASS"),
  TRAP_DOOR("OAK_TRAPDOOR"),
  MONSTER_EGGS("INFESTED_STONE"),
  SMOOTH_BRICK("STONE_BRICKS"),
  HUGE_MUSHROOM_1("BROWN_MUSHROOM_BLOCK"),
  HUGE_MUSHROOM_2("RED_MUSHROOM_BLOCK"),
  IRON_FENCE("IRON_BARS"),
  THIN_GLASS("GLASS_PANE"),
  MELON_BLOCK("MELON"),
  PUMPKIN_STEM("PUMPKIN_STEM"),
  MELON_STEM("MELON_STEM"),
  VINE("VINE"),
  FENCE_GATE("OAK_FENCE_GATE"),
  BRICK_STAIRS("BRICK_STAIRS"),
  SMOOTH_STAIRS("STONE_BRICK_STAIRS"),
  MYCEL("MYCELIUM"),
  WATER_LILY("LILY_PAD"),
  NETHER_BRICK("NETHER_BRICKS"),
  NETHER_FENCE("NETHER_BRICK_FENCE"),
  NETHER_BRICK_STAIRS("NETHER_BRICK_STAIRS"),
  NETHER_WARTS("NETHER_WART"),
  ENCHANTMENT_TABLE("ENCHANTING_TABLE"),
  BREWING_STAND("BREWING_STAND"),
  CAULDRON("CAULDRON"),
  ENDER_PORTAL("END_PORTAL"),
  ENDER_PORTAL_FRAME("END_PORTAL_FRAME"),
  ENDER_STONE("END_STONE"),
  DRAGON_EGG("DRAGON_EGG"),
  REDSTONE_LAMP_OFF("REDSTONE_LAMP"),
  REDSTONE_LAMP_ON("REDSTONE_LAMP"),
  WOOD_DOUBLE_STEP("OAK_SLAB"),
  WOOD_STEP("OAK_SLAB"),
  COCOA("COCOA"),
  SANDSTONE_STAIRS("SANDSTONE_STAIRS"),
  EMERALD_ORE("EMERALD_ORE"),
  ENDER_CHEST("ENDER_CHEST"),
  TRIPWIRE_HOOK("TRIPWIRE_HOOK"),
  TRIPWIRE("TRIPWIRE"),
  EMERALD_BLOCK("EMERALD_BLOCK"),
  SPRUCE_WOOD_STAIRS("SPRUCE_STAIRS"),
  BIRCH_WOOD_STAIRS("BIRCH_STAIRS"),
  JUNGLE_WOOD_STAIRS("JUNGLE_STAIRS"),
  COMMAND("COMMAND_BLOCK"),
  BEACON("BEACON"),
  COBBLE_WALL("COBBLESTONE_WALL"),
  FLOWER_POT("POTTED_CACTUS"),
  CARROT("CARROTS"),
  POTATO("POTATOES"),
  WOOD_BUTTON("OAK_BUTTON"),
  SKULL("AIR"),
  ANVIL("ANVIL"),
  TRAPPED_CHEST("TRAPPED_CHEST"),
  GOLD_PLATE("LIGHT_WEIGHTED_PRESSURE_PLATE"),
  IRON_PLATE("HEAVY_WEIGHTED_PRESSURE_PLATE"),
  REDSTONE_COMPARATOR_OFF("COMPARATOR"),
  REDSTONE_COMPARATOR_ON("COMPARATOR"),
  DAYLIGHT_DETECTOR("DAYLIGHT_DETECTOR"),
  REDSTONE_BLOCK("REDSTONE_BLOCK"),
  QUARTZ_ORE("NETHER_QUARTZ_ORE"),
  HOPPER("HOPPER"),
  QUARTZ_BLOCK("QUARTZ_BLOCK"),
  QUARTZ_STAIRS("QUARTZ_STAIRS"),
  ACTIVATOR_RAIL("ACTIVATOR_RAIL"),
  DROPPER("DROPPER"),
  STAINED_CLAY("WHITE_TERRACOTTA"),
  STAINED_GLASS_PANE("WHITE_STAINED_GLASS_PANE"),
  LEAVES_2("ACACIA_LEAVES"),
  LOG_2("ACACIA_LOG"),
  ACACIA_STAIRS("ACACIA_STAIRS"),
  DARK_OAK_STAIRS("DARK_OAK_STAIRS"),
  SLIME_BLOCK("SLIME_BLOCK"),
  BARRIER("BARRIER"),
  IRON_TRAPDOOR("IRON_TRAPDOOR"),
  PRISMARINE("PRISMARINE"),
  SEA_LANTERN("SEA_LANTERN"),
  HAY_BLOCK("HAY_BLOCK"),
  CARPET("WHITE_CARPET"),
  HARD_CLAY("TERRACOTTA"),
  COAL_BLOCK("COAL_BLOCK"),
  PACKED_ICE("PACKED_ICE"),
  DOUBLE_PLANT("SUNFLOWER"),
  STANDING_BANNER("WHITE_BANNER"),
  WALL_BANNER("AIR"),
  DAYLIGHT_DETECTOR_INVERTED("DAYLIGHT_DETECTOR"),
  RED_SANDSTONE("RED_SANDSTONE"),
  RED_SANDSTONE_STAIRS("RED_SANDSTONE_STAIRS"),
  DOUBLE_STONE_SLAB2("RED_SANDSTONE_SLAB"),
  STONE_SLAB2("RED_SANDSTONE_SLAB"),
  SPRUCE_FENCE_GATE("SPRUCE_FENCE_GATE"),
  BIRCH_FENCE_GATE("BIRCH_FENCE_GATE"),
  JUNGLE_FENCE_GATE("JUNGLE_FENCE_GATE"),
  DARK_OAK_FENCE_GATE("DARK_OAK_FENCE_GATE"),
  ACACIA_FENCE_GATE("ACACIA_FENCE_GATE"),
  SPRUCE_FENCE("SPRUCE_FENCE"),
  BIRCH_FENCE("BIRCH_FENCE"),
  JUNGLE_FENCE("JUNGLE_FENCE"),
  DARK_OAK_FENCE("DARK_OAK_FENCE"),
  ACACIA_FENCE("ACACIA_FENCE"),
  SPRUCE_DOOR("SPRUCE_DOOR"),
  BIRCH_DOOR("BIRCH_DOOR"),
  JUNGLE_DOOR("JUNGLE_DOOR"),
  ACACIA_DOOR("ACACIA_DOOR"),
  DARK_OAK_DOOR("DARK_OAK_DOOR"),
  IRON_SPADE("IRON_SHOVEL"),
  IRON_PICKAXE("IRON_PICKAXE"),
  IRON_AXE("IRON_AXE"),
  FLINT_AND_STEEL("FLINT_AND_STEEL"),
  APPLE("APPLE"),
  BOW("BOW"),
  ARROW("ARROW"),
  COAL("COAL"),
  DIAMOND("DIAMOND"),
  IRON_INGOT("IRON_INGOT"),
  GOLD_INGOT("GOLD_INGOT"),
  IRON_SWORD("IRON_SWORD"),
  WOOD_SWORD("WOODEN_SWORD"),
  WOOD_SPADE("WOODEN_SHOVEL"),
  WOOD_PICKAXE("WOODEN_PICKAXE"),
  WOOD_AXE("WOODEN_AXE"),
  STONE_SWORD("STONE_SWORD"),
  STONE_SPADE("STONE_SHOVEL"),
  STONE_PICKAXE("STONE_PICKAXE"),
  STONE_AXE("STONE_AXE"),
  DIAMOND_SWORD("DIAMOND_SWORD"),
  DIAMOND_SPADE("DIAMOND_SHOVEL"),
  DIAMOND_PICKAXE("DIAMOND_PICKAXE"),
  DIAMOND_AXE("DIAMOND_AXE"),
  STICK("STICK"),
  BOWL("BOWL"),
  MUSHROOM_SOUP("MUSHROOM_STEW"),
  GOLD_SWORD("GOLDEN_SWORD"),
  GOLD_SPADE("GOLDEN_SHOVEL"),
  GOLD_PICKAXE("GOLDEN_PICKAXE"),
  GOLD_AXE("GOLDEN_AXE"),
  STRING("STRING"),
  FEATHER("FEATHER"),
  SULPHUR("GUNPOWDER"),
  WOOD_HOE("WOODEN_HOE"),
  STONE_HOE("STONE_HOE"),
  IRON_HOE("IRON_HOE"),
  DIAMOND_HOE("DIAMOND_HOE"),
  GOLD_HOE("GOLDEN_HOE"),
  SEEDS("WHEAT_SEEDS"),
  WHEAT("WHEAT"),
  BREAD("BREAD"),
  LEATHER_HELMET("LEATHER_HELMET"),
  LEATHER_CHESTPLATE("LEATHER_CHESTPLATE"),
  LEATHER_LEGGINGS("LEATHER_LEGGINGS"),
  LEATHER_BOOTS("LEATHER_BOOTS"),
  CHAINMAIL_HELMET("CHAINMAIL_HELMET"),
  CHAINMAIL_CHESTPLATE("CHAINMAIL_CHESTPLATE"),
  CHAINMAIL_LEGGINGS("CHAINMAIL_LEGGINGS"),
  CHAINMAIL_BOOTS("CHAINMAIL_BOOTS"),
  IRON_HELMET("IRON_HELMET"),
  IRON_CHESTPLATE("IRON_CHESTPLATE"),
  IRON_LEGGINGS("IRON_LEGGINGS"),
  IRON_BOOTS("IRON_BOOTS"),
  DIAMOND_HELMET("DIAMOND_HELMET"),
  DIAMOND_CHESTPLATE("DIAMOND_CHESTPLATE"),
  DIAMOND_LEGGINGS("DIAMOND_LEGGINGS"),
  DIAMOND_BOOTS("DIAMOND_BOOTS"),
  GOLD_HELMET("GOLDEN_HELMET"),
  GOLD_CHESTPLATE("GOLDEN_CHESTPLATE"),
  GOLD_LEGGINGS("GOLDEN_LEGGINGS"),
  GOLD_BOOTS("GOLDEN_BOOTS"),
  FLINT("FLINT"),
  PORK("PORKCHOP"),
  GRILLED_PORK("COOKED_PORKCHOP"),
  PAINTING("PAINTING"),
  GOLDEN_APPLE("GOLDEN_APPLE"),
  SIGN("SIGN"),
  WOOD_DOOR("OAK_DOOR"),
  BUCKET("BUCKET"),
  WATER_BUCKET("WATER_BUCKET"),
  LAVA_BUCKET("LAVA_BUCKET"),
  MINECART("MINECART"),
  SADDLE("SADDLE"),
  IRON_DOOR("IRON_DOOR"),
  REDSTONE("REDSTONE"),
  SNOW_BALL("SNOWBALL"),
  BOAT("OAK_BOAT"),
  LEATHER("LEATHER"),
  MILK_BUCKET("MILK_BUCKET"),
  CLAY_BRICK("BRICK"),
  CLAY_BALL("CLAY_BALL"),
  SUGAR_CANE("SUGAR_CANE"),
  PAPER("PAPER"),
  BOOK("BOOK"),
  SLIME_BALL("SLIME_BALL"),
  STORAGE_MINECART("CHEST_MINECART"),
  POWERED_MINECART("FURNACE_MINECART"),
  EGG("EGG"),
  COMPASS("COMPASS"),
  FISHING_ROD("FISHING_ROD"),
  WATCH("CLOCK"),
  GLOWSTONE_DUST("GLOWSTONE_DUST"),
  RAW_FISH("COD"),
  COOKED_FISH("COOKED_COD"),
  INK_SACK("INK_SAC"),
  BONE("BONE"),
  SUGAR("SUGAR"),
  CAKE("CAKE"),
  BED("RED_BED"),
  DIODE("REPEATER"),
  COOKIE("COOKIE"),
  MAP("FILLED_MAP"),
  SHEARS("SHEARS"),
  MELON("MELON_SLICE"),
  PUMPKIN_SEEDS("PUMPKIN_SEEDS"),
  MELON_SEEDS("MELON_SEEDS"),
  RAW_BEEF("BEEF"),
  COOKED_BEEF("COOKED_BEEF"),
  RAW_CHICKEN("CHICKEN"),
  COOKED_CHICKEN("COOKED_CHICKEN"),
  ROTTEN_FLESH("ROTTEN_FLESH"),
  ENDER_PEARL("ENDER_PEARL"),
  BLAZE_ROD("BLAZE_ROD"),
  GHAST_TEAR("GHAST_TEAR"),
  GOLD_NUGGET("GOLD_NUGGET"),
  NETHER_STALK("NETHER_WART"),
  POTION("POTION"),
  GLASS_BOTTLE("GLASS_BOTTLE"),
  SPIDER_EYE("SPIDER_EYE"),
  FERMENTED_SPIDER_EYE("FERMENTED_SPIDER_EYE"),
  BLAZE_POWDER("BLAZE_POWDER"),
  MAGMA_CREAM("MAGMA_CREAM"),
  BREWING_STAND_ITEM("BREWING_STAND"),
  CAULDRON_ITEM("CAULDRON"),
  EYE_OF_ENDER("ENDER_EYE"),
  SPECKLED_MELON("GLISTERING_MELON_SLICE"),
  MONSTER_EGG("PIG_SPAWN_EGG"),
  EXP_BOTTLE("EXPERIENCE_BOTTLE"),
  FIREBALL("FIRE_CHARGE"),
  BOOK_AND_QUILL("WRITABLE_BOOK"),
  WRITTEN_BOOK("WRITTEN_BOOK"),
  EMERALD("EMERALD"),
  ITEM_FRAME("ITEM_FRAME"),
  FLOWER_POT_ITEM("FLOWER_POT"),
  CARROT_ITEM("CARROT"),
  POTATO_ITEM("POTATO"),
  BAKED_POTATO("BAKED_POTATO"),
  POISONOUS_POTATO("POISONOUS_POTATO"),
  EMPTY_MAP("MAP"),
  GOLDEN_CARROT("GOLDEN_CARROT"),
  SKULL_ITEM("SKELETON_SKULL"),
  CARROT_STICK("CARROT_ON_A_STICK"),
  NETHER_STAR("NETHER_STAR"),
  PUMPKIN_PIE("PUMPKIN_PIE"),
  FIREWORK("FIREWORK_ROCKET"),
  FIREWORK_CHARGE("FIREWORK_STAR"),
  ENCHANTED_BOOK("ENCHANTED_BOOK"),
  REDSTONE_COMPARATOR("COMPARATOR"),
  NETHER_BRICK_ITEM("NETHER_BRICK"),
  QUARTZ("QUARTZ"),
  EXPLOSIVE_MINECART("TNT_MINECART"),
  HOPPER_MINECART("HOPPER_MINECART"),
  PRISMARINE_SHARD("PRISMARINE_SHARD"),
  PRISMARINE_CRYSTALS("PRISMARINE_CRYSTALS"),
  RABBIT("RABBIT"),
  COOKED_RABBIT("COOKED_RABBIT"),
  RABBIT_STEW("RABBIT_STEW"),
  RABBIT_FOOT("RABBIT_FOOT"),
  RABBIT_HIDE("RABBIT_HIDE"),
  ARMOR_STAND("ARMOR_STAND"),
  IRON_BARDING("IRON_HORSE_ARMOR"),
  GOLD_BARDING("GOLDEN_HORSE_ARMOR"),
  DIAMOND_BARDING("DIAMOND_HORSE_ARMOR"),
  LEASH("LEAD"),
  NAME_TAG("NAME_TAG"),
  COMMAND_MINECART("COMMAND_BLOCK_MINECART"),
  MUTTON("MUTTON"),
  COOKED_MUTTON("COOKED_MUTTON"),
  BANNER("BLACK_BANNER"),
  SPRUCE_DOOR_ITEM("SPRUCE_DOOR"),
  BIRCH_DOOR_ITEM("BIRCH_DOOR"),
  JUNGLE_DOOR_ITEM("JUNGLE_DOOR"),
  ACACIA_DOOR_ITEM("ACACIA_DOOR"),
  DARK_OAK_DOOR_ITEM("DARK_OAK_DOOR"),
  GOLD_RECORD("MUSIC_DISC_13"),
  GREEN_RECORD("MUSIC_DISC_CAT"),
  RECORD_3("MUSIC_DISC_BLOCKS"),
  RECORD_4("MUSIC_DISC_CHIRP"),
  RECORD_5("MUSIC_DISC_FAR"),
  RECORD_6("MUSIC_DISC_MALL"),
  RECORD_7("MUSIC_DISC_MELLOHI"),
  RECORD_8("MUSIC_DISC_STAL"),
  RECORD_9("MUSIC_DISC_STRAD"),
  RECORD_10("MUSIC_DISC_WARD"),
  RECORD_11("MUSIC_DISC_11"),
  RECORD_12("MUSIC_DISC_WAIT");

  private String fallback;

  EnumMaterial(String fallback) {
    this.fallback = fallback;
  }

  /**
   * @return Pega o nome do field aplicado na classe {@link Material}.
   */
  public String getName() {
    return MinecraftVersion.getCurrentVersion().getCompareId() >= 1131 ? this.fallback : this.name();
  }

  public Material toBukkitMaterial() {
    return Material.matchMaterial(this.getName());
  }

  // Armazenar, loopar o "values" todas as vezes demora muito.
  private static final EnumMaterial[] VALUES;

  static {
    VALUES = EnumMaterial.values();
  }

  /**
   * Procura por um field {@code Material} a partir do nome aplicado na classe {@link Material}.
   *
   * @param nameToMatch Nome para procurar.
   * @return Retorna o Material Bukkit ou nulo caso não encontre.
   */
  public static Material matchMaterial(String nameToMatch) {
    for (EnumMaterial material : VALUES) {
      if (material.getName().equalsIgnoreCase(nameToMatch)) {
        return material.toBukkitMaterial();
      }
    }

    // Volta para o matchMaterial do próprio bukkit.
    return Material.matchMaterial(nameToMatch);
  }
}

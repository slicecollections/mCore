...
public class MenuBoosters extends PlayerMenu {
  ...
  public MenuBoosters(Profile profile) {
    ...
    for (int index = 0; index < Core.minigames.size(); index++) {
      ...
      network.append(" &8• &b" + minigame + ": "
          + (nb == null ? "&cDesativado" : "&6" + nb.getMultiplier() + "x &7por " + Role.getColored(nb.getBooster()) + " &8(" + TimeUtils.getTimeUntil(nb.getExpires()) + ")")
          + (index + 1 == Core.minigames.size() ? "" : "\n"));
    }
    ...
    if (booster == null) {
      ...
      result.append("&8• &6Multiplicador " + splitted[0] + "x &8(" + TimeUtils.getTimeUntil(Long.parseLong(splitted[1]))
          + ")\n \n&fCálculo:\n &7Com o multiplicador ativo ao receber &650 Coins &7o\n &7total recebido será equivalente a &6" + (int) all + " Coins&7.");
    }

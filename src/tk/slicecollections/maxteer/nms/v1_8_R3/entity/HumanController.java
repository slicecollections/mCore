...
public class HumanController extends AbstractEntityController {
  ...
  protected Entity createEntity(Location location, NPC npc) {
    ...
    EntityNPCPlayer handle = new EntityNPCPlayer(location, nmsWorld.getMinecraftServer(), nmsWorld, profile, new PlayerInteractManager(nmsWorld), npc);

    handle.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
    Bukkit.getScheduler().scheduleSyncDelayedTask(NPCLibrary.getPlugin(), new Runnable() {
